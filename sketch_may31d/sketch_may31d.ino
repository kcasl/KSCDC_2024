...
int DISPLAY_VERSION = 0;                            // Which display should be used for the remote controller? 1D or 2D version?
String gar;
int THROTTLE_MINIMUM = 1000;                        // Minimum throttle of a motor
int THROTTLE_MAXIMUM = 2000;                        // Maximum throttle of a motor
...
float gain_p[3] = {0.8, 0.8, 0.5};                    // Gain proportional
float gain_i[3] = {0.5, 0.5, 0.5};                        // Gain integral
...
void setup() {
  ...
  /* Attach the motors */
  motor_1.attach(2,THROTTLE_MINIMUM, THROTTLE_MAXIMUM);                                
  motor_2.attach(3,THROTTLE_MINIMUM, THROTTLE_MAXIMUM);
  motor_3.attach(4,THROTTLE_MINIMUM, THROTTLE_MAXIMUM);
  motor_4.attach(5,THROTTLE_MINIMUM, THROTTLE_MAXIMUM);
...
void loop() {
  ...
  if(throttle > 1010) {
    ...
    if (throttle < 1150) setSpeedForAllMotors(throttle);
    else setMotorPids();
  } else {
    /* Turn off all motors */
...
void calculatePid() {
  ...
  error_current[PITCH] = angle_current[PITCH]+1.15 - angle_desired[PITCH];
  error_current[ROLL] = angle_current[ROLL]+2.27 - angle_desired[ROLL];
  error_current[YAW] = angle_current[YAW]-1.42 - angle_desired[YAW];
      ...
      if (length == 4 && temp != 0) {
        if (temp == 1000) {
          throttle = 1000;
            ...
            Serial.println(temp);
            throttle = constrain(temp,throttle-10,throttle+10);
...
void sendData2() {
  Serial.println("B2" +
    // String(throttle) + "|" + 
    // String(error_current[PITCH]) + "|" + 
    // String(error_current[ROLL]) + "|" + 
    // String(error_current[YAW]) + "|" + 
